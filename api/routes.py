...
def update():
    ...
    # print((camera_data[current[0]["name"]]["latitude"], camera_data[current[0]["name"]]["longitude"]))
    ...
    for i in range(3):
        ...
        with inference_lock, torch.inference_mode():
            results = model.predict(
                source=frame,
                device=device,
                workers=0,      # no dataloader threads
                stream=False,
                verbose=False
                # imgsz=640, conf=0.25, iou=0.45   # (optional)
            )
        # print(2)
...
def stream(idx):
    while True:
        # while camera_frames[idx] == None:
        #     print(camera_frames[idx])
...
def get_optimal_lanes():
    ...
    return jsonify((data - np.min(data)) / (np.max(data) - np.min(data))), 200
